...
**Admin settings (Settings module):** admins manage role-based accounts — create, delete, **reset passwords**, and **edit roles** (`/api/users`, `PATCH /api/users/:id/role`) — plus **email invitations** with one-time setup links (`/api/invites`: tokens are hashed server-side, revocable, valid 7 days, single-use, accepted via `?invite=<token>` on the login page). They also configure log/data retention and the **password policy** (`passwordMaxAgeDays`; 0 = never expire) via `/api/settings`, set/rotate the Gemini API key at runtime (no restart, stored server-side, never returned), and view the **live audit trail** (append-only, streamed over SSE; `GET /api/audit`, admin only).

**Password lifecycle:** a complexity policy (min 8 chars, upper/lower/digit/special, no username) is enforced at creation, admin reset, self-service change (`POST /api/auth/change-password`), and invite acceptance. When `passwordMaxAgeDays > 0`, logins with expired passwords receive `403 PASSWORD_EXPIRED` and must rotate via `POST /api/auth/expired-password` before using the platform.
